...
function History(props) {
  ...
  const linkDirect =
    type === "topup"
      ? `${isMobile}/dashboard/topup/status?order_id=${id}`
      : `${isMobile}/dashboard/transfer/status/${id}`;
  return (
    <div
      className={`d-flex align-items-center justify-content-between py-3 bg-white rounded-14 ${
        flat ? "" : "shadow-sm px-3"
      } ${className}`}
    >
      <div className="d-flex align-items-center">
        ...
        />
        <div className="pl-3">
          <Link className="font-weight-bold text-dark" to={linkDirect}>
            {name}
          </Link>
